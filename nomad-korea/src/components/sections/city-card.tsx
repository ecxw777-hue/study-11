...
export function CityCard({ city, view = "grid" }: CityCardProps) {
  if (view === "list") {
    return (
      <Link href={`/cities/${city.slug}`} className="block">
        <article className="group flex flex-row overflow-hidden rounded-xl border border-border bg-surface transition-all hover:border-nk-accent/30 hover:shadow-lg hover:shadow-nk-accent/5">
          {/* Image */}
          ...
          <div className="flex flex-1 flex-col justify-between space-y-2 p-4">
            <div>
              <div className="mb-2 flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-bold text-foreground">{city.nameKo}</h3>
                  <p className="text-sm text-dim">{city.name}</p>
                </div>
